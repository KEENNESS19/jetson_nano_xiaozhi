...
setup_opus()
try:
    import smbus  # 传统smbus
except ImportError:
    from smbus2 import SMBus as smbus  # 备用smbus2
# 配置日志
...
class Application:
    ...
    def _initialize_audio(self):
        try:
            logger.debug("初始化音频编解码器和语音识别模块")
            from src.audio_codecs.audio_codec import AudioCodec
            self.audio_codec = AudioCodec()
            
            # 初始化语音识别模块为录音模式
            self.bus = smbus.SMBus(1)  # I2C 总线
            self.i2c_addr = 0x0f       # 模块地址
            self.bus.write_byte_data(self.i2c_addr, 0x02, 0x02)  # 设置为非关键词模式
            logger.info("音频设备和语音识别模块初始化完成")
        except Exception as e:
    ...
    def _handle_input_audio(self):
        ...
        # 从语音识别模块读取原始音频
        raw_audio = self.read_raw_audio()
        if raw_audio and self.protocol.is_audio_channel_opened():
            asyncio.run_coroutine_threadsafe(
                self.protocol.send_audio(raw_audio),
                self.loop
            )
    
    def read_raw_audio(self):
        audio_data = []
        for _ in range(1024):
            audio_data.append(self.bus.read_byte_data(self.i2c_addr, 0x10))  # 假设 0x10 是音频数据寄存器
    ...
    def _initialize_iot_devices(self):
        """初始化物联网设备"""
        from src.iot.thing_manager import ThingManager
        from src.iot.things.rider import ArmController
        # 获取物联网设备管理器实例
        thing_manager = ThingManager.get_instance()

        # 添加设备
        thing_manager.add_thing(ArmController())
